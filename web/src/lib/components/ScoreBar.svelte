<script lang="ts">
    ...
    // White's share of the bar (0 to 1)
    ...
    const labelSide = $derived(() => whiteShare() > 0.5 ? 'black' : 'white')
</script>

<div class="bar-wrap">
    <div class="bar">
        <div class="white-seg" style="height: {whiteShare() * 100}%"></div>
        <div class="black-seg" style="height: {(1 - whiteShare()) * 100}%"></div>
    </div>
    <span class="label {labelSide()}">{label()}</span>
</div>
...
<style>
    ...
    .black-seg { background: #1a1a1a; transition: height 0.3s ease; flex: 1; }
    .label {
        position: absolute;
        font-size: 9px;
    ...
    .label.black { top: 6px; color: rgba(255,255,255,0.7); }
    .label.white { bottom: 6px; color: rgba(0,0,0,0.5); }
</style>
