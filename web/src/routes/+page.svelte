                        ...
                        <div class="move-card">
                            ...
                            <div class="move-card-stats">
                                <div class="stat">
                                    ...
                                    <span class="stat-value">{selectedMove.loss_cp}cp</span>
                                </div>
                                <div class="stat">
                                    <span class="stat-label">Score</span>
                                    <span class="stat-value">{selectedMove.score_cp !== null ? (selectedMove.score_cp > 0 ? '+' : '') + selectedMove.score_cp : 'M'}</span>
                                </div>
                                <div class="stat">
                                    <span class="stat-label">Rank</span>
                                    <span class="stat-value">{selectedMove.rank !== null ? '#' + selectedMove.rank : '—'}</span>
                                </div>
                                {#if selectedMove.best_uci && selectedMove.rank !== 1}
                                    <div class="stat">
                                        <span class="stat-label">Best was</span>
                                        <span class="stat-value mono">{selectedMove.best_uci}</span>
                                    </div>
                                {/if}
                            </div>
                        </div>
                    {:else}
                        <p class="select-hint">Click any move or bar to inspect it</p>
                    {/if}
                </div>
...
<style>
    ...
    .move-san { font-size: 1.4rem; font-weight: 700; color: #fff; font-family: monospace; }
    .move-class {
        font-size: 0.72rem;
    ...
    .move-card-stats { display: flex; flex-direction: column; gap: 0.4rem; }
    .stat { display: flex; justify-content: space-between; align-items: baseline; }
    .stat-label { font-size: 0.72rem; color: rgba(255,255,255,0.3); }
    .stat-value { font-size: 0.85rem; color: #ddd; font-family: monospace; }
